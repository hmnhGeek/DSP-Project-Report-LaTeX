...
def step1(image, message, block_size=5, dont_delete=False):
    # generate message bits.
    mbits = generateMessage(message)
    ...
    for t in t_range:
        ...
        # finding argmax.
        if (2*len(EU) >= len(mbits)):
            Final_EU = EU
            T = t
    # the set EU has been decided now, EU is the Final_EU. This set already includes those pixels
    # which are greater than T.
