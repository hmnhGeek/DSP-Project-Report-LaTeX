...
import merge_images.merge_images
import pickle
import lsb_matching.lsb_matching
...
def pixelate(image, message, block_size=5, dont_delete=False):
    ...
    merge_images.merge_images.merge_folder(os.getcwd(), block_size, new_width, new_height)
    ...
    key = lsb_matching.lsb_matching.image_embedding(scrumbled_image_name, message, 'final.'+image_name+'-stego.png')
    print(key)
...
def decode(stego_image, key_file):
    ...
    key = key.split(' ')[1]
    msg = lsb_matching.lsb_matching.expose_message('final.final.png', int(key))
